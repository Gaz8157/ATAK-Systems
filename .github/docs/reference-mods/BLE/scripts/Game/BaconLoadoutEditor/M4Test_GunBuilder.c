...
sealed enum Bacon_GunBuilderUI_Mode {
	CHARACTER,
	ENTITY,
	STORAGE,
	OPTIONS,
...
sealed enum Bacon_GunBuilderUI_InputMode {
	VIEW_SLOTS,
	SWAP_SLOT,
	VIEW_OPTIONS,
...
sealed enum Bacon_GunBuilderUI_StorageType {
	DEFAULT,
	CHARACTER_LOADOUT,
	CHARACTER_EQUIPMENT,
	CHARACTER_WEAPON,
...
sealed enum Bacon_GunBuilderUI_SlotType {
	CHARACTER_LOADOUT,
	CHARACTER_WEAPON,
	CHARACTER_EQUIPMENT,
...
sealed class Bacon_GunBuilderUI_SlotIdentity: Bacon_GunBuilderUI_SlotInfo {
	string factionKey;
	// int factionIdentityId;
	// int componentRplId;
}

sealed class Bacon_GunBuilderUI_ItemListing {
	int numItems;
	Bacon_GunBuilderUI_SlotInfo slotInfo;
}

sealed class Bacon_GunBuilderUI_SlotInfoStorageItem: Bacon_GunBuilderUI_SlotInfo {
	int numItems = 1;
}
...
sealed class Bacon_GunBuilderUI_SlotLoadout: Bacon_GunBuilderUI_SlotItem {
...
sealed class Bacon_GunBuilderUI_SlotChoiceIdentity: Bacon_GunBuilderUI_SlotChoice {
...
sealed class Bacon_GunBuilderUI: ChimeraMenuBase {
	...
	GunBuilderUI_RankAndSupplyInfoComponent m_RankAndSupplyComponent;
	...
	void ShowIntroDialog() {
		...
		SCR_ConfigurableDialogUi dialog = Bacon_GunBuilderUI_Helpers.CreateDialog("intro", "This update introduces saving rank in loadouts - this means that previously saved loadouts will no longer load. Roll back to previous version if you wish to retain them. Otherwise remove the folder named 1.3.\n\nBefore you report bugs and/or crashes, make sure you can reproduce them WITHOUT ANY OTHER MODS LOADED!!\n\nIf you experience issues while having other mods loaded, contact authors of those mods.");
	...
	void Init(IEntity arsenalEntity, IEntity characterEntity) {
		...
		if (!m_inventoryManager || !m_arsenalComponent || !m_pcComponent || !m_pInputmanager) { 
			OnCriticalError(Bacon_GunBuilderUI_Step.MENU_INIT, "Init:2: null variable", ""); return; }
		...
		m_RankAndSupplyComponent = GunBuilderUI_RankAndSupplyInfoComponent.Cast(m_root.FindAnyWidget("RankAndSupplyInfo").FindHandler(GunBuilderUI_RankAndSupplyInfoComponent));
...
//			return;
	...
	void OnEditedSlotChanged(GunBuilderUI_SlotsUIComponent comp, Managed data, int button) {
		...
		Bacon_GunBuilderUI_SlotChoiceIdentity slotChoiceIdentity = Bacon_GunBuilderUI_SlotChoiceIdentity.Cast(data);
	...
	void HandleClickedInventoryItem(Bacon_GunBuilderUI_SlotInfoStorageItem itemInfo, int button) {
		...
		request.arsenalEntityRplId = Bacon_GunBuilderUI_Helpers.GetEntityRplId(m_ArsenalEntity);
...
//		Print("Listbox child id: "+selected.listBoxChildId);
	...
	void CreateIdentitySlot() {
		SCR_CharacterIdentityComponent identityComponent = SCR_CharacterIdentityComponent.Cast(m_EditedEntity.FindComponent(SCR_CharacterIdentityComponent));
		
		// InventoryStorageSlot storageSlot = storage.GetSlot(storageSlotId);

		Bacon_GunBuilderUI_SlotIdentity slotInfo = new Bacon_GunBuilderUI_SlotIdentity();

		if (!Bacon_GunBuilderUI_Helpers.ValidateIdentitySlot(identityComponent, slotInfo)) {
		...
		// slotInfo.storageType = Bacon_GunBuilderUI_StorageType.CHARACTER_IDENTITY;
		// slotInfo.slot = null;

		// slotInfo.listBoxChildId = m_wSlotChoicesListbox.AddItem("storageSlot", slotInfo);
		if (m_arsenalFaction)
			slotInfo.factionKey = m_arsenalFaction.GetFactionKey();
		slotInfo.slotType = Bacon_GunBuilderUI_SlotType.CHARACTER_VISUAL_IDENTITY;
		slotInfo.listBoxChildId = m_wSlotChoicesListbox.AddItem_Choice(slotInfo);
	}
...
//		if (ChimeraCharacter.Cast(m_EditedEntity)) {
//			CreateIdentitySlot();
//		}
	...
	void HandleChangedIdentity(Bacon_GunBuilderUI_SlotChoiceIdentity slotChoice) {
		m_iListBoxLastActionChild = slotChoice.listBoxChildId;
		
		Bacon_GunBuilderUI_Network_StorageRequest request = new Bacon_GunBuilderUI_Network_StorageRequest();
//		Bacon_GunBuilderUI_Network_IdentityChangeRequest request = new Bacon_GunBuilderUI_Network_IdentityChangeRequest();
		
		request.arsenalEntityRplId = Bacon_GunBuilderUI_Helpers.GetEntityRplId(m_ArsenalEntity);
		request.storageSlotId = slotChoice.factionIdentityId;

		request.actionType = Bacon_GunBuilderUI_ActionType.CHANGE_VISUAL_IDENTITY;
		
//		request.storageRplId = m_Cache.GetStorageRplId(editedSlot.slot.GetStorage());
//		request.storageSlotId = slotChoice.storageSlotId;
...
//	}
	...
	void OnServerResponse_Loadout(Bacon_GunBuilderUI_Network_Response response, Bacon_GunBuilderUI_Network_LoadoutRequest request) {
		...
		switch (request.actionType) {
			...
			case Bacon_GunBuilderUI_ActionType.APPLY_LOADOUT: {
				HandleMessage(true, response.message);
				Bacon_GunBuilderUI_Helpers.PlaySoundForSlotType(Bacon_GunBuilderUI_SlotType.CHARACTER_LOADOUT);
				break;
			}
		}
	...
	void RefreshUpdatedSlot(bool success, RplId storageRplId, int slotId) {
		...
		if (success) {
			...
			Bacon_GunBuilderUI_Helpers.PlaySoundForSlot(slotInfo);
		}
	...
	void CreateRemoveSlotOption(Bacon_GunBuilderUI_SlotChoice slotInfo) {
		// PrintFormat("Name: %1", slotInfo.slotName);
	...
	void CreateOptionsForSlot() {
		...
		if (slotInfo.slotType == Bacon_GunBuilderUI_SlotType.CHARACTER_VISUAL_IDENTITY) {
			numChoices = CreateOptionsForVisualIdentity(Bacon_GunBuilderUI_SlotIdentity.Cast(slotInfo));
		} else {
			array<ResourceName> prefabChoices = {};
			numChoices = m_Cache.GetChoicesForSlotType(slotInfo, prefabChoices);
	...
	int CreateOptionsForVisualIdentity(Bacon_GunBuilderUI_SlotIdentity slotInfo) {
		// ResourceName attachedItemResourceName;
		//Bacon_GunBuilderUI_Helpers.GetResourceNameFromEntity(slotInfo.slot.GetAttachedEntity(), attachedItemResourceName);
		...
		array<ref VisualIdentity> options = {};
		int numChoices = m_Cache.GetVisualIdentityChoicesForFaction(slotInfo, options);
		
		if (numChoices < 1) {
			return 0;
		}
		
		string factionName = m_arsenalFaction.GetFactionName();
		
		foreach (int index, VisualIdentity identity : options) {
			Bacon_GunBuilderUI_SlotChoiceIdentity optionSlotInfo = new Bacon_GunBuilderUI_SlotChoiceIdentity();
			
			optionSlotInfo.factionIdentityId = index;
			
			optionSlotInfo.prefab = identity.GetHead();
			optionSlotInfo.prefabBody = identity.GetBody();
			
			optionSlotInfo.slotName = string.Format("%1 Faction", factionName);
			optionSlotInfo.itemName = string.Format("Visual Identity %1", index);
			
			optionSlotInfo.slotType = Bacon_GunBuilderUI_SlotType.CHARACTER_VISUAL_IDENTITY;
