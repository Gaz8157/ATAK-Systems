...
sealed class GunBuilderUI_CategoryButtonUIComponent: SCR_ButtonImageComponent {
	...
	void SetCategoryName(string data) { m_sCategoryName = data; }
	string GetCategoryName() { return m_sCategoryName; }
...
//		m_wButtonRight.SetVisible(state);
	...
	void VisualizeSelection() {
		foreach (GunBuilderUI_CategoryButtonUIComponent handler : m_wButtonComponents) {
			if (handler.GetCategoryName() == m_sCurrentCategory) {
	...
	void OnButtonClicked(SCR_ButtonBaseComponent comp) {
		...
		string selected = button.GetCategoryName();
