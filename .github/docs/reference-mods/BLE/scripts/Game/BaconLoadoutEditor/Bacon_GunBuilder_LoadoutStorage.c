sealed class Bacon_GunBuilder_PlayerLoadout {
...
sealed class Bacon_GunBuilder_PlayerFactionLoadoutStorage {
	...
	static bool FillPlayerLoadoutMetadata(IEntity ent, out string outClothes, out string outWeapons, out string outRank) {
		...
		if (itemsInLoadout.Count() > 0) {
			...
			foreach (IEntity item : itemsInLoadout) {
				...
				rank = Bacon_GunBuilderUI_Helpers.GetItemRequiredRankFromCache(prefab);
	...
	bool SaveLoadout(SCR_ArsenalManagerComponent arsenalManager, IEntity characterEntity, string factionKey, int slotId) {
		...
		if (!Bacon_GunBuilder_PlayerFactionLoadoutStorage.FillPlayerLoadoutMetadata(characterEntity, newPlayerLoadout.metadata_clothes, newPlayerLoadout.metadata_weapons, newPlayerLoadout.required_rank))
			return false;
		...
		// supply stuff
		if (factionKey != "admin") {
			SCR_Faction faction = Bacon_GunBuilderUI_Helpers.GetFactionFromFactionKey(factionKey);
			if (!faction) {
				Print(string.Format("Bacon_GunBuilder_PlayerFactionLoadoutStorage | Invalid faction '%1'", factionKey), LogLevel.ERROR);
				return false;
			}
			
			float cost;
			if (!arsenalManager.GunBuilderUI_GetLoadoutRespawnCost(newPlayerLoadout.data, faction, cost)) {
				Print("Bacon_GunBuilder_PlayerFactionLoadoutStorage | Could not compute loadout cost :(", LogLevel.ERROR);
				return false;
			}
...
sealed class Bacon_GunBuilder_LoadoutStorageComponentClass: SCR_BaseGameModeComponentClass {}

sealed class Bacon_GunBuilder_LoadoutStorageComponent: SCR_BaseGameModeComponent {
	// key: identityid
	...
	string loadoutPathRoot = "$profile:/BaconLoadoutEditor_Loadouts/1.4";
	...
	bool SavePlayerLoadoutToFile(int playerId, string identityId, string factionKey, bool isAdminLoadout = false) {
		if (!Bacon_GunBuilderUI_Helpers.IsFileSavingEnabled())
			return true;
