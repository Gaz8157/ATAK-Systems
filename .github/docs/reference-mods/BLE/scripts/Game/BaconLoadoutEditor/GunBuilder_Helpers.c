sealed class Bacon_GunBuilderUI_Helpers {
	...
	static ref map<ResourceName, ref Bacon_GunBuilderUI_Cache_ArsenalItemDetails> m_ArsenalItemDetails = new map<ResourceName, ref Bacon_GunBuilderUI_Cache_ArsenalItemDetails>;
...
//	static RplId GetComponentRplId(IEntity ent) {
//		RplComponent rpl = RplComponent.Cast(ent.FindComponent(RplComponent));
//		if (!rpl)
//			return RplId.Invalid();
//		
...
//	}
	static SCR_Faction GetFactionFromFactionKey(string factionKey) {
		...
		if (!fm) {
			Print(string.Format("Bacon_GunBuilderUI_Helpers | No faction manager"), LogLevel.ERROR);
			return null;
		}
		SCR_Faction faction = SCR_Faction.Cast(fm.GetFactionByKey(factionKey));
		if (!faction) {
			Print(string.Format("Bacon_GunBuilderUI_Helpers | Invalid faction '%1'", factionKey), LogLevel.ERROR);
		}
	...
	static SCR_ArsenalInventoryStorageManagerComponent GetArsenalComponent(IEntity entity) {
		if (!entity)
			return null;

		return SCR_ArsenalInventoryStorageManagerComponent.Cast(entity.FindComponent(SCR_ArsenalInventoryStorageManagerComponent));
	}
	
	static BaseInventoryStorageComponent FindAnyStorageComponent(IEntity entity) {
		if (!entity)
			return null;

		return BaseInventoryStorageComponent.Cast(entity.FindComponent(BaseInventoryStorageComponent));
	}
	
	static void FillSupplyAndRankCache(SCR_ArsenalItem arsenalItem) {
		Bacon_GunBuilderUI_Cache_ArsenalItemDetails details = new Bacon_GunBuilderUI_Cache_ArsenalItemDetails;
	...
	static float GetItemSupplyCost(SCR_ArsenalComponent arsenalComponent, ResourceName prefab) {
		...
		Bacon_GunBuilderUI_Cache_ArsenalItemDetails details = m_ArsenalItemDetails.Get(prefab);
		if (details) {
	...
	static bool AreSuppliesEnabled(IEntity arsenalEntity) {
		...
		return AreSuppliesEnabled(resourceComponent);
	}
	
	static bool AreSuppliesEnabled(SCR_ResourceComponent resourceComponent) {
...
//		Print(string.Format("slot: %1", slot));
	...
	static SCR_ConfigurableDialogUi CreateDialog(string dialogType, string dialogContent, string debugInformation = "") {
		...
		if (!debugInformation.IsEmpty()) {
			...
			m_dialog.GetRootWidget().FindAnyWidget("DebugMessagePanel").SetVisible(true);
		}
	...
	static bool ValidateResourceName(ResourceName resourceName) {
	...
	static bool ValidateIdentitySlot(SCR_CharacterIdentityComponent identityComponent, out Bacon_GunBuilderUI_SlotIdentity slotInfo) {
		slotInfo.slotName = "Visual Identity";
		slotInfo.itemName = "Unknown Identity";
		...
		if (!ValidateResourceName(visualIdentity.GetHead())) {
			slotInfo.itemName = "Invalid head prefab";
			return false;
		}
		
		if (!ValidateResourceName(visualIdentity.GetBody())) {
			slotInfo.itemName = "Invalid body prefab";
			return false;
		}
	...
	static void PlaySoundForSlot(Bacon_GunBuilderUI_SlotInfo slot) {
		switch (slot.slotType) {
			case Bacon_GunBuilderUI_SlotType.CHARACTER_LOADOUT: {
				string nameLower = slot.slotName;
				nameLower.ToLower();
				if (nameLower.Contains("velcro")) {
					SCR_UISoundEntity.SoundEvent("BACON_SOUND_VELCRO");
					break;
				}
				
				SCR_UISoundEntity.SoundEvent("BACON_SOUND_LOADOUT");
				break;
			}
			case Bacon_GunBuilderUI_SlotType.CHARACTER_WEAPON: {
				SCR_UISoundEntity.SoundEvent("BACON_SOUND_WEAPON");
				break;
			}
			case Bacon_GunBuilderUI_SlotType.CHARACTER_EQUIPMENT: {
				SCR_UISoundEntity.SoundEvent("BACON_SOUND_EQUIPMENT");
				break;
			}
			case Bacon_GunBuilderUI_SlotType.ATTACHMENT: {
				SCR_UISoundEntity.SoundEvent("BACON_SOUND_ATTACHMENT");
				break;
			}
			case Bacon_GunBuilderUI_SlotType.MAGAZINE: {
				SCR_UISoundEntity.SoundEvent("BACON_SOUND_MAGAZINE");
				break;
			}
			case Bacon_GunBuilderUI_SlotType.OPTION: {
				SCR_UISoundEntity.SoundEvent("BACON_SOUND_OPTION");
				break;
			}
		}
	}
	...
	static void PlaySound(string type) {
		switch (type) {
			case "blocked": {
				SCR_UISoundEntity.SoundEvent("BACON_SOUND_BLOCKED");
				break;
			}
		}
	}
