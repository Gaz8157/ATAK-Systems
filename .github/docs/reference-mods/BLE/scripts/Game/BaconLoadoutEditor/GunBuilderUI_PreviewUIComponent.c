...
sealed class GunBuilderUI_PreviewUIComponent : ScriptedWidgetComponent {
	...
	static IEntity m_LightEntities;
	...
	// static ResourceName m_PreviewWorldResourceName = "{28F15B2438B4B16A}Prefabs/BaconLoadoutEditor/PreviewWorld/GunBuilderPreviewWorldPS5.et";
    // static ResourceName m_PreviewWorldResourceName = "{28F15B2438B4B16A}Prefabs/BaconLoadoutEditor/PreviewWorld/GunBuilderPreviewWorldPS5.et";
	
	// static ResourceName m_PreviewWorldResourceName = "{9DD3ABD459AB6120}Prefabs/BaconLoadoutEditor/PreviewWorld/GunBuilderPreviewWorld_Festive.et";
	...
	protected void CreatePreview()
	{
		...
		// m_LightEntities = m_PreviewWorld.FindEntityByName("LightEntities_1");
		
		// m_LightAnglesTarget = m_LightEntities.GetAngles(); 

		m_wRenderTarget.SetWorld(m_PreviewWorld, m_PreviewCamera);
		GetGame().GetCallqueue().CallLater(SpawnLight, 500, false);
	}
	...
	void SpawnLight() {
		...
		Resource rsc = Resource.Load(m_PreviewWorldLightsResourceName);
		if (rsc.IsValid())
			GetGame().SpawnEntityPrefab(rsc, m_PreviewWorld);
...
//		vector pMins, pMaxs, pCenter;
//		m_PreviewEntity.GetBounds(pMins, pMaxs);
//		pCenter = vector.Lerp(pMins, pMaxs, 0.5);
		...
		if (m_previewMode == Bacon_GunBuilderUI_Mode.CHARACTER) {
			...
			if (slotInfo.slotType == Bacon_GunBuilderUI_SlotType.CHARACTER_LOADOUT) {
				...
				if (areaType == LoadoutBootsArea) {
					...
					// @hack
					// if center is very low then use slot position
					// if (center[1] < 0.1) {
					vector slotMat[4];
					slotInfo.slot.GetWorldTransform(slotMat);
			
					targetCameraLookPosition = m_PreviewEntity.GetOrigin() + editedEntity.CoordToLocal(slotMat[3]) + center;
					// }
					...
					if (areaType == LoadoutBackpackArea) {
						...
						// test 2
						// vector wtf = editedEntity.CoordToLocal(slotMat[3]);
						if (center[2] < -0.15) {
							targetCameraDirection = vector.Direction(targetCameraLookPosition, targetCameraLookPosition-(vector.Forward + vector.Right));
							return true;
						}
//						if (targetCameraLookPosition[0] < 0 || targetCameraLookPosition[2] < 0) {
//							targetCameraDirection = vector.Direction(targetCameraLookPosition, targetCameraLookPosition-(vector.Forward + vector.Right));
//							return true;
//						}
		...
		if (m_previewMode == Bacon_GunBuilderUI_Mode.ENTITY) {
			...
			// targetCameraLookPosition = pCenter + editedEntity.CoordToLocal(slotMat[3]) + vector.Lerp(mins, maxs, 0.5);
...
sealed class Bacon_TestClass: ScriptComponentClass {}
sealed class Bacon_Test: ScriptComponent {
