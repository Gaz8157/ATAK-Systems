class GunBuilderUI_SlotsUIComponent: ScriptedWidgetComponent {
	...
	sealed int AddItem_Slot(Bacon_GunBuilderUI_SlotInfo slotInfo) {
		return AddItem(slotInfo, m_sResourceName_SlotItem);
	}
	
	sealed int AddItem_Choice(Bacon_GunBuilderUI_SlotChoice slotInfo) {
		return AddItem(slotInfo, m_sResourceName_SlotItem);
	}
	
	sealed int AddItem_Loadout(Bacon_GunBuilderUI_SlotLoadout slotInfo) {
		return AddItem(slotInfo, m_sResourceName_LoadoutItem);
	}
	
	sealed int AddItem_InventoryItem(Bacon_GunBuilderUI_SlotInfoStorageItem slotInfo) {
		return AddItem(slotInfo, m_sResourceName_InventoryItem);
	}
	
	sealed int AddItem_Option(GunBuilderUI_EditorOptionData optionData) {
		return AddItem(optionData, m_sResourceName_InventoryItem);
	}
	
	sealed void SetFocusedItemSelected(int button = 0) {
		if (m_focused == -1)
			return;

		OnClicked(m_aElementComponents[m_focused], button);
	}
	sealed void SetFocus(int child) {
		if (child >= m_aElementComponents.Count())
			return;

		GetGame().GetCallqueue().CallLater(_SetFocusDelayed, 33, false, child);
	}
	sealed void SetFocusByZOrder(int z) {
	...
	sealed GunBuilderUI_MultifunctionSlotUIComponent GetItem(int childId) {
		return m_aElementComponents[childId];
	}
	
	sealed int AddItem(Managed data, ResourceName layout) {
