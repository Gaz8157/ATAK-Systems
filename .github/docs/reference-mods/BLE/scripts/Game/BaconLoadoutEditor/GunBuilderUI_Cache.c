sealed class Bacon_GunBuilderUI_Cache_ArsenalItemDetails {
	float supplyCost;
	SCR_ECharacterRank requiredRank;
}

sealed class Bacon_GunBuilderUI_Cache {
	private static Bacon_GunBuilderUI_Cache m_Instance;
	static Bacon_GunBuilderUI_Cache GetInstance() { return m_Instance; }
	...
	// private ref map<ResourceName, SCR_ECharacterRank> m_ArsenalItemRank = new map<ResourceName, SCR_ECharacterRank>;
	
	private ref map<ResourceName, ref Bacon_GunBuilderUI_Cache_ArsenalItemDetails> m_ArsenalItemDetails = new map<ResourceName, ref Bacon_GunBuilderUI_Cache_ArsenalItemDetails>;
	...
	bool Init(SCR_ArsenalComponent arsenalComponent) {
		...
		// foreach (
		// CachePrefabCost(arsenalItem.GetItemResourceName(), arsenalItem.GetSupplyCost(SCR_EArsenalSupplyCostType.DEFAULT));
	...
	bool AreSuppliesEnabled() {
		return Bacon_GunBuilderUI_Helpers.AreSuppliesEnabled(m_ArsenalResourceComponent);
	}
...
//				continue;
	...
	int GetChoicesForSlotType(Bacon_GunBuilderUI_SlotInfo slotInfo, out array<ResourceName> outValidPrefabs) {
		...
		Print(string.Format("Bacon_GunBuilderUI_Cache.GetChoicesForSlotType | Fetched %1 items", items), LogLevel.DEBUG);
		
		return items;
	};
	
	int GetVisualIdentityChoicesForFaction(Bacon_GunBuilderUI_SlotIdentity slotInfo, out array<ref VisualIdentity> outVisualIdentities) {
		// string slotType = slotInfo.slot.Type().ToString();
		...
		if (!faction) {
			PrintFormat("Bacon_GunBuilderUI_Cache.GetVisualIdentityChoicesForFaction | Invalid faction for faction key %1", slotInfo.factionKey, level: LogLevel.ERROR);
			return 0;
		}
		
		FactionIdentity factionIdentity = faction.GetFactionIdentity();
		if (!factionIdentity) {
			PrintFormat("Bacon_GunBuilderUI_Cache.GetVisualIdentityChoicesForFaction | Invalid faction identity for faction key %1", slotInfo.factionKey, level: LogLevel.ERROR);
			return 0;
		}
		
		// array<VisualIdentity> visualIdentities;
		factionIdentity.GetVisualIdentities(outVisualIdentities);
		
		Print(string.Format("Bacon_GunBuilderUI_Cache.GetVisualIdentityChoicesForFaction | Fetched %1 items", outVisualIdentities.Count()), LogLevel.DEBUG);
